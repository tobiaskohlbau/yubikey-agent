...
func main() {
	...
	flag.Usage = func() {
		...
		fmt.Fprintf(os.Stderr, "\n")
		fmt.Fprintf(os.Stderr, "\tyubikey-agent -l NAME\n")
		fmt.Fprintf(os.Stderr, "\n")
		fmt.Fprintf(os.Stderr, "\t\tRun the agent, listening on the named pipe at NAME.\n")
		fmt.Fprintf(os.Stderr, "\n")
	}

	socketPath := flag.String("l", "", "agent: name of the named pipe to listen on")
...
func runAgent(socketPath string) {
	...
	if err != nil {
		log.Fatalln("Failed to listen on named pipe socket:", err)
	}
